...
export const ProductItem = ({item, key}) => {
    ...
    return (
        <div key={key} className="flex flex-col justify-center items-start p-2 bg-white">
            <div className="relative">
                <img className="hidden lg:block" src={item ? item.img : "https://i.ibb.co/4ZPL5F0/Rectangle-37.png"} alt="watch" />
                <img className="lg:hidden" src={item ? item.img : "https://i.ibb.co/4ZPL5F0/Rectangle-37.png"} alt="watch" />
                ...
                <div className="flex justify-between mt-4">
                    <div>
                        <p className="text-lg font-medium leading-none text-gray-800">{item ? item.name : "Sony Digital Watch"}</p>
                    </div>
                    <div>
                        <p className="text-lg leading-none text-right text-gray-600">{item ? item.price : "$1245"}</p>
                    </div>
                </div>
            </div>
        </div>
